...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (empty($_POST['slug']) || empty($_POST['name'])) {
    ...
    // 提交 ID 就代表是更新，则更新数据
    $sql = sprintf("update categories set slug = '%s', name = '%s' where id = %d", $_POST['slug'], $_POST['name'], $_GET['id']);
    // 响应结果
    $message = xiu_execute($sql) > 0 ? '修改成功' : '修改失败';
  }
}
...
if ($_GET['id']){
    $id=$_GET['id'];
    $current_edit_category=xiu_fetch_one(sprintf("select * from categories where id='%d'",$id));
}
...
<body>
  ...
  <div class="main">
    ...
    <div class="container-fluid">
      ...
      <div class="alert alert-<?php echo ($message == '保存成功' || $message='修改成功' ) ? 'success' : 'danger'; ?>">
        <strong><?php echo ($message == '保存成功' || $message='修改成功' )? '成功' : '错误'; ?>！</strong><?php echo $message; ?>
      </div>
      <?php endif; ?>
      <div class="row">
        <div class="col-md-4">
...
<!--            blank form-->
        ...
        <div class="col-md-8">
          <div class="page-action">
            ...
            <a class="btn btn-danger btn-sm btn-delete" href="category-delete.php" style="display: none">批量删除</a>
          </div>
            ...
            <tbody>
              ...
              <tr data-id="<?php echo $item['id']; ?>">
                ...
                <td class="text-center">
                  <a href="categories.php?id=<?php echo $item['id']; ?>" class="btn btn-info btn-xs btn-edit">编辑</a>
  ...
  <script>
      $(function () {
          var checkedbox=$('tbody input[type=checkbox]');
          ...
          checkedbox.on('change',function () {
                ...
                checked.length>0?  deleteBtn.fadeIn(): deleteBtn.fadeOut();
                deleteBtn.prop('search','?id='+ checked);
          });
          ...
          $('thead input').on('change',function () {
              ...
              //触发事件
              checkedbox.prop('checked',checked).trigger('change');
          })
      })
  //  $(function () {
  //    // 获取所需操作的界面元素
